...
> 調査日: 2026-03-12 | ブランチ: `dev` | 現バージョン: v1.6.0
>
> 16エージェントによる並列調査に基づく、piper-plusの今後のロードマップです。
...
| PR | 内容 | 状態 |
|----|------|------|
| #222 | Zero-Shot TTS (Dual-Mode Speaker Conditioning) | Draft |
| #219 | 6言語マルチリンガル音素化 (JA/EN/ZH/ES/FR/PT) | Draft |
| #218 | バイリンガル (JA+EN) TTS v3学習完了 | Open |

### 未着手のオープンIssue

| Issue | 内容 | カテゴリ |
|-------|------|---------|
| #206 | 離散埋め込みによるProsody Embedding強化 | 品質向上 |
| #205 | OpenJTalkフルコンテキストラベル抽出 | 品質向上 |
| #203 | 日本語アクセント強度レベル [1/2/3] | 品質向上 |
| #202 | Duration正則化の追加 | 品質向上 |
| #201 | データ拡張 (SpecAugment, 速度/ピッチ変動) | 品質向上 |
| #200 | Multi-Resolution STFT Discriminator | 品質向上 |
| #153 | Godot向けライブラリ | プラットフォーム |
| #142/#141/#137 | Web デモに中国語/韓国語追加 | 多言語 |
| #110 | 無音・文間のランダム性制御 | 品質向上 |
| #23 | マルチ言語モデルの学習/推論コード対応 | 多言語 |
...
├─ Noise-scaled MAS           ├─ 多言語C++推論         ├─ Few-shot適応        └─ 超軽量モデル
├─ 品質評価ツール・API        └─ 多言語WebUI           └─ ONNX Speaker Enc.
└─ 既存Issue消化
```
...
| 7 | MRSD追加 (Issue #200) | MOS +0.05-0.10 (※学習時間増に注意) | 1-2週間 |
...
| Issue | 内容 | 工数 |
|-------|------|------|
| #200 | Multi-Resolution STFT Discriminator | 1-2週間 |
| #201 | SpecAugment + 速度/ピッチ変動データ拡張 | 1-2週間 |
| #202 | Duration正則化 | 1週間 |
| #206 | 離散Prosody Embedding | 1週間 |
| #110 | 無音・文間ランダム性制御 | 数日 |
